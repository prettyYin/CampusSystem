package com.xiaoyi.management.util;
...
public class UploadFile {
	...
	public static Map<String, Object> getUploadResult(MultipartFile photo, String dirPaht, String portraitPath) {
		...
		if (!photo.isEmpty() && photo.getSize() > 0) {
			...
			error_result = UploadFile.uploadPhoto(photo, dirPaht);
			if (error_result != null && error_result.size() != 0) {
				return error_result;
			}
			//使用UUID重命名图片名称(uuid__原始图片名称)
			String newPhotoName = UUID.randomUUID().toString().replace("-","").toLowerCase()
					+ "__" + orginalName;
			//将上传的文件保存到目标目录下
			try {
